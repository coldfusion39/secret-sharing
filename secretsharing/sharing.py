...
def point_to_share_string(point, charset, num_leading_zeros):
    ...
    if not (isinstance(point, tuple) and len(point) == 2 and
            isinstance(point[0], integer_types) and isinstance(point[1], integer_types)):
...
class SecretSharer():
    ...
    def split_secret(cls, secret_string, share_threshold, num_shares):
        ...
        shares = []
        for point in points:
            share_string = point_to_share_string(point, cls.share_charset, num_leading_zeros)
    ...
    def recover_secret(cls, shares):
        ...
        points = []
        for share in shares:
            point, num_leading_zeros = share_string_to_point(share, cls.share_charset)
            points.append(point)
...
class BitcoinToB58SecretSharer(SecretSharer):
    """ Good for converting Bitcoin secret keys into shares that can be reliably printed out in any font. """
    secret_charset = base58_chars
    share_charset = base58_chars


class BitcoinToB32SecretSharer(SecretSharer):
    """ Good for converting Bitcoin secret keys into shares that can be reliably and conveniently transcribed. """
    secret_charset = base58_chars
